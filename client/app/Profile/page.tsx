...
import { ICreateStorage, IEditStorage, IGetStorageData, IGetStorageWareHouseData, IGetWareHouseByItemTypeApollo } from "@/interfaces/IStorage";
...
import { CREATE_WAREHOUSE_FOR_STORAGE, DELETE_WAREHOUSE_BY_ID, EDIT_WAREHOUSE_ID, GET_WAREHOUSE_BY_ITEM_TYPE } from "@/services/queries";
...
const Profile: React.FunctionComponent = () => {
  ...
  //   /**
  //    *
  //    * @param Queries on fetching data and mutuation for create, edit and delete storage sub items
  //    * GraphQl  Queries
  //    *
  //    */

  //   // For getting storage data by item type in warehouse format
  //   const { data, loading, refetch }: QueryResult<IGetWareHouseByItemTypeApollo, OperationVariables> = useQuery(GET_WAREHOUSE_BY_ITEM_TYPE, { variables: { user_id: userId, item_type: itemName }, skip: userId === null && (itemName === null || itemName === undefined) });

  //   // For Creating a storage
  //   const [createStorage, { loading: createLoad }] = useMutation(CREATE_WAREHOUSE_FOR_STORAGE);

  //   // For Editing a storage
  //   const [editStorage, { loading: editLoad }] = useMutation(EDIT_WAREHOUSE_ID);

  //   // For Deleting a storage
  //   const [deleteStorage, { loading: deleteLoad }] = useMutation(DELETE_WAREHOUSE_BY_ID);

  //   useEffect(() => {
  //     if (itemtype) {
  //       const id = getLocalStorageItem(LOCALUSER);
  //       setUserId(id);
  ...
  //   useEffect(() => {
  //     setSubItems(data?.StoragesByUserIdAndItemName);
  //   }, [data]);
  ...
  //     deleteStorage({
  //       variables: { ware_house_id: id },
  //       onCompleted() {
  //         refetch();
  //         handleEditStorageId(null);
  //         handleDeleteModal();
  //         alert("Storage is Deleted");
  //       },
  //       onError(error) {
  ...
  return (
    <>
      {/* {(loading || createLoad || editLoad || deleteLoad) && <AppLoader />} */}
      <h3 className="text-dark mt-4 mb-5 fw-bold">{`${itemtype}`}</h3>
      {/*
       *
       *
